...
VS Code 스타일 검색/실행 팔레트로 Task 검색, 명령 실행, 뷰 전환을 빠르게 수행합니다.

| 항목 | 설명 |
|------|------|
| 모듈 | `js/commandPalette.js` |
| 단축키 | `Ctrl+P` (토글), `Esc` (닫기) |
| 프리픽스 | (없음)=Task, `/`=명령, `@`=팀원, `.`=뷰 |
| Task 검색 | 퍼지 매칭 → SmartFilter 해제 + 프로젝트 필터 + 하이라이트 |
| 명령 실행 | 각 모듈의 open/toggle 메서드 호출 (raw DOM 조작 금지) |
| 네비게이션 | `App.selectProjectFilterOption(filterValue)` 사용 |
| Arrow 키 | 위아래 순환 (modulo), Enter 실행 |

### App Startup 최적화
Electron 앱 시작 시 흰 화면 방지를 위한 `show:false` + IPC 패턴입니다.

| 항목 | 설명 |
|------|------|
| 핵심 | `BrowserWindow` `show: false` → init 후 IPC `app-ready` → `show()` |
| Paint 보장 | double `requestAnimationFrame` 후 IPC 전송 |
| Fallback | 5초 safety timeout (IPC 실패 방지) |
| Script 로딩 | `index.html` 모든 로컬 script에 `defer` |
| 구현 | `electron-main.js`, `electron-preload.js`, `js/main.js` |
